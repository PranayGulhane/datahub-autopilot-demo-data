...
| **Detected** | 2026-08-03 04:25 UTC+0000 |
...
The `raw.orders` dataset has undergone a structural change where the column previously named `order_total` has been renamed to `order_amount`. This change means that any downstream consumers, such as dashboards, queries, or pipelines that explicitly select the `order_total` column by name, will break and require updates to reference the new column name. This change is not simply a catalog update, but a fundamental change to the dataset's structure that affects how data is accessed.

## Why this is critical

This schema change is warranted a critical severity due to its impact on 3 business-critical assets in tier1, which are directly used by multiple downstream consumers. The high criticality of tier1 assets, such as staging.orders_enriched and dashboards.revenue_overview, amplifies the risk of this change. If unmanaged, it could lead to significant data inconsistencies and revenue loss.
...
A critical schema migration for the raw.orders dataset is required to maintain data integrity, impacting 6 key consumers including staging.orders_enriched and dashboards.customer_360. Failure to execute this migration will result in inaccurate customer lifetime value and revenue metrics, necessitating immediate attention from owners of affected downstream assets.
...
Targets: `staging.orders_enriched`, `marts.customer_ltv`, `marts.revenue_daily`, `marts.order_facts`, `dashboards.customer_360`, `dashboards.revenue_overview`, `marts.product_performance`, `dashboards.product_insights`
...
Targets: `staging.orders_enriched`, `marts.customer_ltv`, `marts.revenue_daily`, `marts.order_facts`, `dashboards.customer_360`, `dashboards.revenue_overview`
