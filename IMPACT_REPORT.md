...
| **Detected** | 2026-08-02 15:56 UTC+0000 |
...
The column 'order_total' has been renamed to 'order_amount' in the 'raw.orders' dataset. This change requires downstream consumers (dashboards, queries, and pipelines) to update any references to 'order_total' to 'order_amount' to avoid errors. Any queries or pipelines that specifically select the 'order_total' column by name will break and need to be revised.

## Why this is critical

A critical schema change to raw.orders poses significant business risk due to its impact on 8 downstream assets, with 3 business-critical tier1 assets affected. This change has the potential to disrupt key business functions, affecting revenue and customer relationships. The high number of affected assets and critical tier1 dependencies warrant a critical risk severity classification.
...
A critical schema migration plan for the raw.orders dataset is required to address breaking changes affecting 6 confirmed consumers, including key business intelligence assets like staging.orders_enriched and dashboards.customer_360. If not addressed, this change will cause data inconsistencies and potential revenue loss. The first step is to notify owners of the affected downstream assets.
...
Targets: `staging.orders_enriched`, `marts.order_facts`, `marts.customer_ltv`, `marts.revenue_daily`, `dashboards.revenue_overview`, `dashboards.customer_360`, `marts.product_performance`, `dashboards.product_insights`
...
Targets: `staging.orders_enriched`, `marts.order_facts`, `marts.customer_ltv`, `marts.revenue_daily`, `dashboards.revenue_overview`, `dashboards.customer_360`
