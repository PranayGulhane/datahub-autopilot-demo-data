...
| **Detected** | 2026-08-02 14:43 UTC+0000 |
...
The 'order_total' column in the 'raw.orders' dataset has been renamed to 'order_amount'. This change will break any downstream consumers that explicitly select the column by its original name, as they will now need to reference it as 'order_amount'.

## Why this is critical

This schema change is critical due to its impact on 8 downstream assets, with 3 tier1 assets, the highest criticality tier, being directly affected. The potential for business disruption is significant, given the business-critical nature of these tier1 assets. If left unmanaged, this change could lead to data inconsistencies and errors in business-critical dashboards and marts.
...
A critical schema migration plan for the raw.orders dataset is required to address breaking changes that will impact 6 downstream assets, including staging.orders_enriched and marts.order_facts. If not addressed, this change will cause data inconsistencies and potential revenue loss. The first step is to notify the owners of the affected assets.
...
Targets: `staging.orders_enriched`, `marts.order_facts`, `marts.revenue_daily`, `marts.customer_ltv`, `dashboards.customer_360`, `dashboards.revenue_overview`, `marts.product_performance`, `dashboards.product_insights`
...
Targets: `staging.orders_enriched`, `marts.order_facts`, `marts.revenue_daily`, `marts.customer_ltv`, `dashboards.customer_360`, `dashboards.revenue_overview`
