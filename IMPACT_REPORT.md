...
| **Detected** | 2026-08-02 16:03 UTC+0000 |
...
Schema change on raw.orders: renamed `order_total` -> `order_amount`. This is breaking: downstream consumers referencing the affected column(s) by name will fail, regardless of how a catalog labels the change structurally.

## Why this is critical

Risk on raw.orders assessed critical: a breaking change with 8 downstream consumer(s), highest criticality tier1. Breaking changes propagate to every consumer that reads the affected column.
...
6-step plan for a breaking change on raw.orders, assessed critical. 6 consumer(s) reference the changed field(s) directly and must be updated first; 2 more need a read-through for indirect references. Regenerate lower-environment fixtures, re-validate, then record the outcome in DataHub.
...
| `total_orders` | CURRENCY_AMOUNT |
| `lifetime_value` | GENERIC_NUMBER |
...
| `avg_order_value` | GENERIC_NUMBER |
