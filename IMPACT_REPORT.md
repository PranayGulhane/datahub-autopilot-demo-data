...
| **Detected** | 2026-08-02 09:14 UTC+0000 |
...
The column 'order_total' has been renamed to 'order_amount' in the 'raw.orders' dataset. This change will break any downstream consumers that select this column by its original name, including dashboards, queries, and pipelines. Any code referencing 'order_total' will need to be updated to use the new name 'order_amount'.

## Why this is critical

A critical schema change to raw.orders is warranted due to its impact on 8 downstream assets, with 3 tier1 assets, the highest criticality tier, being directly affected. This change poses a significant risk to business-critical operations, potentially leading to data inconsistencies and disruptions in dashboards such as customer_360 and revenue_overview, which rely heavily on staging.orders_enriched and marts.revenue_daily.
...
A critical schema migration for the raw.orders dataset is required to prevent data inconsistencies and potential revenue loss. The change affects 6 downstream assets, including key business intelligence dashboards and mart tables, and requires immediate attention from owners of these assets to ensure a smooth transition. Notification of affected owners is the first critical step to prevent data disruptions.
